...
def masked_soft_target_cross_entropy(
    ...
    """Average soft-target cross entropy over all supervised observer rows."""
    ...
    if logits.ndim != 3 or logits.shape[1:] != (NUM_PLAYERS, NUM_PLAYERS):
    ...
    if not torch.isfinite(valid_logits).all():
        raise ValueError("valid logits must contain only finite values")
...
__all__ = ["masked_soft_target_cross_entropy"]
