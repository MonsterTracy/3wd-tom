...
保留的唯一 label pipeline 是：公开发言前冻结边界，读取目标 playing agent 的合法私有 observation，执行 readonly self-report，保存 `suspected_werewolves` 符号集合，再由唯一 Dataset 确定性转换为 7×7 observer-conditioned belief target。
